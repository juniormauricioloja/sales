...
class Products extends Admin_Controller
{
    ...
    public function get_product_inventory(){
      ...
      if($result){
        echo json_encode(array('message'=>'Información del Inventario','type'=>'success',"data"=>$result));
      }else {
        echo json_encode(array('message'=>'Algo salió mal, intentalo de nuevo','type'=>'warning'));
      }
    }
    public function add()
    {
    ...
    {//var_dump($this->input->post());die;
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          if($result){
            ...
            $this->session->set_flashdata(array('message'=>'Producto Añadido Satisfactoriamente','type'=>'success'));
          }else {
            $this->session->set_flashdata(array('message'=>'Algo salió mal. Intentalo de nuevo','type'=>'warning'));
          }
    ...
    public function edit($prod_id)
    {
          ...
          if ($row) {
            ...
            $this->session->set_flashdata(array('message'=>'No se encontraron los registros','type'=>'warning'));
            redirectToAdmin('products');
          }
    ...
    public function edit_products($prod_id){
     ...
     $result=$this->products_model->edit($prod_id,$data_to_update);
      ...
      if($result){
        $this->session->set_flashdata(array('message'=>'Producto actualizado satisfactoriamente','type'=>'success'));
      }else {
        $this->session->set_flashdata(array('message'=>'Algo salió mal. Intentalo de nuevo','type'=>'warning'));
      }
      redirectToAdmin('products');
    ...
    public function delete()
    {
      ...
      if($result){
        ...
        echo json_encode(array('message'=>'Producto eliminado satisfactoriamente','type'=>'success'));
      }else {
        echo json_encode(array('message'=>'Algo salió mal. Intentalo de nuevo','type'=>'warning'));
      }
    }
